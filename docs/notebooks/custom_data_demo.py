...
# play around. On the GitHub it is in [docs/notebooks](./notebooks/extended_demo.py)
# . To run this on your
...
#
# Since this is a notebook, we will define [`config`](config_guide.md)
...
# [filepath](reference-filepaths.md), which are the files generated by the current
# workflow if verbosity is true (i.e., `swmmanywhere.logging.set_verbose(True)`).
#
# You can also use `address_overrides` to you add your own custom data that
# might be used by a [custom graphfcn](graphfcns_guide.md), for example. If you are
# adding new data to the SWMManywhere workflow in this way, you will have to access it
# from the `FilePaths` object with the `get_path` method.
#
