...
def test_replace_parameter_group():
    ...
    class NewTopologyDerivation(parameters.TopologyDerivation):
        new_weight: float = parameters.Field(default = 1,
            le = 1,
            ge = 0,
        )

    # Check has new_weight
    assert hasattr(parameters.get_full_parameters()["topology_derivation"], "new_weight")

    # Check retained existing weight
    assert hasattr(parameters.get_full_parameters()["topology_derivation"], "chahinian_angle_scaling")