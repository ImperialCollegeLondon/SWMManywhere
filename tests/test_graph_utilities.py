...
def test_identify_outlets_no_river(street_network):
    ...
    addresses = parameters.FilePaths(
        base_dir=None, project_name=None, bbox_number=None, model_number=None
    )
...
def test_iterate_graphfcns_noedges():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert not (
            addresses.model / "remove_non_pipe_allowable_links_graph.json"
        ).exists()
...
def test_clip_to_catchments(street_network):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        temp_path = Path(temp_dir)
        addresses = parameters.FilePaths(
            base_dir=temp_path, project_name="test", bbox_number=0, model_number=0
        )
