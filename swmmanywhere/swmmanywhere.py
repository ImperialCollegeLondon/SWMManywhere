...
def swmmanywhere(config: dict) -> tuple[Path, dict | None]:
    ...
    addresses = preprocessing.create_project_structure(
        config["bbox"],
        config["project"],
        config["base_dir"],
    ...
    go.graph_to_geojson(G, addresses.nodes, addresses.edges, G.graph["crs"])
    ...
    synthetic_results = run(addresses.inp, **config["run_settings"])
    logger.info("Writing synthetic results.")
    if verbose():
        synthetic_results.to_parquet(addresses.model / f"results.{addresses.extension}")
    ...
    if config.get("real", {}).get("results", None):
        ...
        if verbose():
            real_results.to_parquet(
                config["real"]["inp"].parent / f"real_results.{addresses.extension}"
            )
    else:
