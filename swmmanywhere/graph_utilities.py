...
def iterate_graphfcns(
    G: nx.Graph, graphfcn_list: list[str], params: dict, addresses: parameters.FilePaths
) -> nx.Graph:
...
class clip_to_catchments(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if verbose():
            subbasins.to_file(
                str(addresses.nodes).replace("nodes", "subbasins"), driver="GeoJSON"
            )
...
class set_elevation(
    ...
    def __call__(
        self, G: nx.Graph, addresses: parameters.FilePaths, **kwargs
    ) -> nx.Graph:
        ...
        elevations = go.interpolate_points_on_raster(x, y, addresses.elevation)
...
class derive_topology(
    BaseGraphFunction,
    required_edge_attributes=["edge_type", "weight"],  # 'rivers' and 'streets'
